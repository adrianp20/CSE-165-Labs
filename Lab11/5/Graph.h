...
public:
    ...
    void add(Vertex* d){
        ...
        items.push_back(d); //adds the vertex to the vector
        d->ref(); // increments the reference count
    }
    ...
    void print(){
        //insert your code here
        for (int i = 0; i < items.size(); i+=2) { // i+=2 because we want to print every other item
            cout << items[i]->getLabel() << " - " << items[i+1]->getLabel() << endl; // print the label of the vertex
        }
    }
    ...
    void deleteAll(){
        //insert your code here
        for (int i = 0; i < items.size(); i++) { //for each vertex
            items[i]->unref(); //unref() is defined in Shared.h
            //delete items[i];
        }
